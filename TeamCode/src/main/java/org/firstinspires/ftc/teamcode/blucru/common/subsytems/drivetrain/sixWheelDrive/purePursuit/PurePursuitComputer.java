...
public class PurePursuitComputer {
    ...
    public double[] computeRotAndXY(Point2d[] path, Pose2d robotPose, Pose2d robotVel, double lookAheadDist,
        ...
        // Calculate final target heading for the PID controller
        // If driving backwards, the PID should target 180 degrees away from the original target
        double finalTargetHeadingDeg = targetHeadingDeg;
        if (isDrivingBackwards) {
            finalTargetHeadingDeg += 180;
        }
        ...
        double rot = pid.getHeadingVelToTargetTurnTo(robotPose, finalTargetHeadingDeg, robotVel.getH());
